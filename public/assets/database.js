export default {
	...
	/* Changed the body properties from an array with paragraphs, to a single string with whitespace. 
	This seemed like a simpler solution for adding line breaks between paragraphs, without needing loops, mapping the array or importing each index in the array
	instead of just body itself (to avoid the square brackets showing up).
	This works because the string is inside a template literal (backticks ``), and because Article.vue has the styling "white-space: pre-line;" which preserves whitespace
	and wraps text (also removes whitespace at start of newlines, from the space left over at the end of the previous paragraph. */
	articles: [
		{
	...
	// The body proportions were also changed here, as the same problems with the brackets occurred. (Changed from an array with multiple paragraps to one string.)
	pages: [
		{
			name: 'About us',
			slug: 'about-us',
